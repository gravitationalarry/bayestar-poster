...
\newflowframe{27cm}{35cm}{88cm}{0cm}
...
\begin{align}
	...
	= 1 - \int
		\left[ 1 - b_1 \left( \gamma_1^{-1} \omega \right) \right]
		\left[ 1 - b_2 \left( \gamma_2^{-1} \omega \right) \right] \nonumber \\ \cdots
		\left[ 1 - b_N \left( \gamma_N^{-1} \omega \right) \right]
...
We compared the detection efficiency of the noncoordinated planner and our two coordinated planners using a set of 2126 GW skymaps from low-mass inspiral signals injected into 24 hours of simulated initial LIGO noise.  Surprisingly, we found that (a) the greedy planner performed almost as well as the anneal planner in terms of detection efficiency, and (b) both coordinated planners had roughly \emph{double} the detection efficiency as the noncoordinated planner.
\end{staticcontents*}
